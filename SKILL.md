...
This skill includes:
- **Tool icons** for common data tools (Snowflake, Airflow, dbt, Kafka, etc.) - see `references/icons.md`
- **Smart discovery mode** to generate diagrams from live Snowflake metadata - see `references/smart-discovery.md`
...
When the diagram involves recognized data tools, use branded icons instead of generic shapes. Consult `references/icons.md` for the full catalog of available icons with ready-to-use style strings.

**Available icons**: Snowflake, Airflow, dbt, Kafka, Apache Spark, Python, AWS S3, Azure, Tableau, Power BI, Looker, Fivetran, Databricks, Sigma, Streamlit

**Usage**: Copy the **Style** string from the icon entry and use it as the `style` attribute on an `mxCell`. The icon renders as an image with the tool label displayed beneath it. Example:

```xml
<mxCell id="sf1" value="Snowflake" style="shape=image;verticalLabelPosition=bottom;verticalAlign=top;imageAspect=0;aspect=fixed;image=data:image/svg+xml,%3Csvg...ENCODED_SVG...%3C%2Fsvg%3E" vertex="1" parent="1">
  <mxGeometry x="100" y="100" width="48" height="48" as="geometry"/>
</mxCell>
```

**IMPORTANT**: Icons use `data:image/svg+xml,` with URL-encoded SVG (NOT base64). The `;` in `data:image/svg+xml;base64,` conflicts with draw.io's style property delimiter.

**When to use icons vs shapes**: Use icons for external tools and platforms (Airflow, dbt, Tableau). Use draw.io native shapes (`cylinder3`, `rounded=1`, etc.) for Snowflake-internal objects like tables, views, and stages.

## Smart discovery mode

When the user wants a diagram based on their **actual Snowflake environment** (not a hypothetical architecture), follow the discovery workflow in `references/smart-discovery.md`:

1. **Scope** - Ask which database(s)/schema(s) to diagram
2. **Discover** - Query Snowflake metadata (tables, views, tasks, streams, stages, dynamic tables, pipes)
3. **Detect patterns** - Identify medallion layers, ETL flows, external tool indicators
4. **Build diagram** - Generate the diagram from discovered objects using icons and proper layout
5. **Annotate gaps** - Add sticky notes for things metadata cannot reveal (external sources, consumers, SLAs, ownership)

This mode produces a "living document" that reflects the real state of the account and highlights what still needs human validation.
